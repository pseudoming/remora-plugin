...
class TestGetSysUptime:
    def test_normal(self):
        from adapter.sandbox.zombie_linux import get_sys_uptime
        mock_file = mock_open(read_data="123.45 67.89\n")
        with patch("adapter.sandbox.zombie_linux.open", mock_file):
            val = get_sys_uptime()
            assert val == 123.45

    def test_exception_returns_zero(self):
        from adapter.sandbox.zombie_linux import get_sys_uptime
        with patch("adapter.sandbox.zombie_linux.open", side_effect=IOError("fail")):
            val = get_sys_uptime()
            assert val == 0.0
...
class TestCleanWhitelist:
    def test_file_not_exists(self, tmp_path):
        from adapter.sandbox.zombie_linux import clean_whitelist
    ...
    def test_stale_pids_cleaned(self, tmp_path):
        ...
        with patch("adapter.sandbox.zombie_linux.os.path.exists", side_effect=mock_exists), \
             patch("adapter.sandbox.zombie_linux.os.makedirs"):
    ...
    def test_read_exception_graceful(self, tmp_path):
        ...
        with patch("adapter.sandbox.zombie_linux.os.path.exists", return_value=True), \
             patch("adapter.sandbox.zombie_linux.open", side_effect=IOError("fail")):
            result = clean_whitelist(str(whitelist_path))
            assert result == set()
