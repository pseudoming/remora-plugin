...
> 安装位置：`~/.gemini/config/plugins/remora-plugin` | 平台依赖：Antigravity Hook / Sidecar 协议 | 557 tests passing
...
- 📓 **决策记忆网络** — 全自动提取会话中的架构决策。FTS5 trigram 中文分词全文索引，三通道混合召回。用户确认后晋升 `manual` 级别，永久免于 GC
...
- 🚧 **全局写门禁** — 首次写入核心代码 → deny + 要求解释意图。二次重试 → allow。relax 模式自适应放行
...
└───┬──────┬──────┬──────┬────────┬────────┬───────────────────────┘
  ...
  │   (mode判定, 决策重载)│ │  check ───►│ │  (制品MD5)  │         │
  │                       │ │  (命令审计) │ │             │         │
  │ action-gate ──────────►│ │            │ │ clean-session-stats  │
  │   (phantom检测)        │ │ cognitive- │ │                       │
  │                       │ │  push ────►│ │ check-subagents-     │
  │ cognitive-push ───────►│ │  (写门禁)   │ │  liveness            │
  │   (决策注射, 写门禁)    │ │            │ │  (子代理探活)         │
  │                       │ │ zombie-    │ │                       │
  │ zombie-detector ──────►│ │  detector  │ └──────────┬────────────┘
  │   (进程扫描)            │ │  (工具前拦截)│           │
  │                       │ └────────────┘            │
  │ tone-injector          │                           │
  │   (语气纪律)            │                           │
...
```bash
# 运行测试
pytest scripts/tests/ -q                         # 557 tests
