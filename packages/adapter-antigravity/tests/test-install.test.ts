...
describe("TestUninstall", () => {
  ...
  it("test_uninstall_starts_without_source", () => {
    const spyFind = vi.spyOn(install, "findPluginRoot").mockImplementation(() => {
      throw new Error("No source directory found");
    });
    const mockMainReal = vi.spyOn(install, "mainReal").mockImplementation(() => {});
  ...
  it("test_workflows_dynamic_cleanup", () => {
    const workflowsSrcDir = path.join(tempRoot, "conf", "templates", "workflows");
    ...
    const globalWorkflowsDir = path.join(mockGeminiConfigDir, "global_workflows");
    ...
    expect(fs.existsSync(path.join(globalWorkflowsDir, "custom_flow.md"))).toBe(false);
    expect(fs.existsSync(path.join(globalWorkflowsDir, "retro.md"))).toBe(false);
  });
  ...
  it("test_workflows_static_cleanup_fallback", () => {
    ...
    expect(fs.existsSync(path.join(globalWorkflowsDir, "confirm.md"))).toBe(false);
    expect(fs.existsSync(path.join(globalWorkflowsDir, "retro.md"))).toBe(false);
    expect(fs.existsSync(path.join(globalWorkflowsDir, "some_other.md"))).toBe(true);
  });

  it("test_project_config_cleanup", () => {
    const projectDir = path.join(mockGeminiConfigDir, "projects");
    fs.mkdirSync(projectDir, { recursive: true });
    const projectFile = path.join(projectDir, "11111111-1111-1111-1111-111111111111.json");
