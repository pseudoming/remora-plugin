...
vi.mock("@remora/core", async (importOriginal) => {
	...
	return {
		...
		getConn: () =>
			new Database(process.env.REMORA_DB_PATH || "", { timeout: 5000 }),
	};
});
...
describe("test_subagent_liveness_and_progress", () => {
	...
	describe("ProgressSentinel.update", () => {
		it("test_progress_sentinel_update", () => {
			...
			const success = ProgressSentinel.update(
				transcriptPath,
				"running",
				5,
				"Initial test step",
			...
			const success2 = ProgressSentinel.update(
				transcriptPath,
				"blocked",
				undefined,
				"Encountered error",
	...
	describe("runAudit", () => {
		...
		it("test_liveness_db_timeout", () => {
			...
			const expiredUtcStr = new Date(Date.now() - 150000)
				.toISOString()
				.replace("T", " ")
				.replace("Z", "");
		...
		it("test_liveness_db_active", () => {
			...
			const freshUtcStr = new Date(Date.now() - 30000)
				.toISOString()
				.replace("T", " ")
				.replace("Z", "");
		...
		it("test_db_blocked_keyword", () => {
			...
			insertMessage(
				...
				1,
				"tool",
				"permission_denied: cannot execute",
				nowStr,
			);
	...
	describe("main — hook mode", () => {
		...
		it("test_liveness_with_watermarks_and_timeframe", () => {
			...
			initDb([
				MESSAGES_SCHEMA,
				WATERMARKS_SCHEMA,
				PROJECT_TOPICS_SCHEMA,
				RUNTIME_HOOK_STATE_SCHEMA,
			]);

			const db = new Database(TEST_DB_PATH, { timeout: 15000 });
			db.prepare(
				"INSERT OR REPLACE INTO watermarks (conversation_id, project_uuid) VALUES (?, ?)",
			).run(parentConvId, projectUuid);
			db.prepare(
				"INSERT OR REPLACE INTO watermarks (conversation_id, project_uuid) VALUES (?, ?)",
			).run(subActive, projectUuid);
			db.prepare(
				"INSERT OR REPLACE INTO watermarks (conversation_id, project_uuid) VALUES (?, ?)",
			).run(subStale, projectUuid);
			db.prepare(
				"INSERT OR REPLACE INTO watermarks (conversation_id, project_uuid) VALUES (?, ?)",
			).run(subWrongProject, "proj_other");

			const nowTs = Date.now();
			const activeTopicTsStr = new Date(nowTs - 50000)
				.toISOString()
				.replace("T", " ")
				.replace("Z", "");
			db.prepare(
			...
			for (let i = 0; i < 25; i++) {
				allMockSteps.push({
					step_index: i,
					timestamp: new Date(nowTs - 100000)
						.toISOString()
						.replace("T", " ")
						.replace("Z", ""),
					content:
						i === 5
							? `Spawned stale subagent conversationId: ${subStale}`
							: `Normal step ${i}`,
				});
			}
			allMockSteps.push({
				step_index: 25,
				timestamp: new Date(nowTs - 30000)
					.toISOString()
					.replace("T", " ")
					.replace("Z", ""),
			...
			vi.spyOn(
				ConversationDataAccessLayer.prototype,
				"getLatestUserMessage",
			).mockReturnValue("heartbeat check");
			vi.spyOn(
				ConversationDataAccessLayer.prototype,
				"getLatestPlannerResponse",
			).mockReturnValue("");
