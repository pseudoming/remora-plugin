...
describe("TestDecodeVarint", () => {
	...
	it("test_eof_empty", () => {
		expect(() => decodeVarint(Buffer.from([]), 0)).toThrow(
			"Unexpected end of data while reading varint",
		);
	});

	it("test_eof_partial", () => {
		expect(() => decodeVarint(Buffer.from([0x80]), 0)).toThrow(
			"Unexpected end of data while reading varint",
		);
	});

	it("test_eof_with_offset", () => {
		expect(() => decodeVarint(Buffer.from([0x08]), 1)).toThrow(
			"Unexpected end of data while reading varint",
		);
	});
...
describe("TestParseProtobuf", () => {
	...
	it("test_multiple_varints_same_tag", () => {
		const result = parseProtobuf(
			Buffer.concat([varintField(1, 10), varintField(1, 20)]),
		);
	...
	it("test_multiple_tags", () => {
		const result = parseProtobuf(
			Buffer.concat([varintField(1, 10), varintField(2, 20)]),
		);
...
describe("TestExtractStepPayload", () => {
	...
	it("test_content_decode_replacement_chars_nodejs", () => {
		...
		const data = lengthField(
			20,
			tag20Blob({ content: Buffer.from([0xff, 0xfe]) }),
		);
		const result = extractStepPayload(data);
	...
	it("test_monologue_decode_success", () => {
		const data = lengthField(
			20,
			tag20Blob({ monologue: Buffer.from("thinking...") }),
		);
		const result = extractStepPayload(data);
	...
	it("test_monologue_decode_replacement_chars_nodejs", () => {
		...
		const data = lengthField(
			20,
			tag20Blob({ monologue: Buffer.from([0xff, 0xfe]) }),
		);
		const result = extractStepPayload(data);
	...
	it("test_tool_calls_valid_json", () => {
		const tc = tcBlob({
			tcId: Buffer.from("call_1"),
			name: Buffer.from("my_tool"),
			args: Buffer.from('{"key":"val"}'),
		});
	...
	it("test_tool_calls_invalid_json", () => {
		const tc = tcBlob({
			tcId: Buffer.from("call_2"),
			name: Buffer.from("another_tool"),
			args: Buffer.from("not-json"),
		});
		const data = lengthField(20, tag20Blob({ toolCalls: [tc] }));
		const result = extractStepPayload(data);
		expect(result["tool_calls"]).toHaveLength(1);
	...
	it("test_full_entry", () => {
		...
		const tc = tcBlob({
			tcId: Buffer.from("call_x"),
			name: Buffer.from("search"),
			args: Buffer.from('{"q":"test"}'),
		});
