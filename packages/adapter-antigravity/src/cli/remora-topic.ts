...
export function main(): void {
	...
	if (!projectUuid) {
		error(
			"Project UUID is required. Please specify via -u/--uuid or ANTIGRAVITY_PROJECT_ID env var.",
		);
		process.exit(1);
	}
	...
	try {
		if (args.action === "new") {
			...
			forceColdStart();
			console.log(
				`Created active topic ${args.name} in project ${projectUuid}.`,
			);
			...
			forceColdStart();
			console.log(
				`Switched active topic to ${args.name} in project ${projectUuid}.`,
			);
			...
			if (!success) {
				warn(
					`No decision found with ID ${args.decisionId} in project ${projectUuid}.`,
				);
			} else {
				console.log(
					`Decision ${args.decisionId} confirmed in project ${projectUuid}.`,
				);
				...
				try {
					...
					if (worktrees.length > 0) {
						...
						if (physicalFiles.length > 0 && tId) {
							...
							console.log(
								`[Remora] Integrated ${physicalFiles.length} physical changed files from sandbox.`,
							);
						}
					} else {
