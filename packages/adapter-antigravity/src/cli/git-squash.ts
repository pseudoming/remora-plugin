...
export function squashCommits() {
  ...
  if (status !== "") {
    throw new Error("Workspace is dirty. Please commit or stash changes before squashing.");
  }

  // 2. Fetch the last commit hash starting with `[Phase`
  const logLimit = 50;
  const logOutput = execSync(`git log -n ${logLimit} --format="%H %s"`, { encoding: "utf-8" });
  ...
  if (!targetHash) {
    throw new Error("Could not find any commit starting with '[Phase' in the last 50 commits.");
  }

  // 3. Collect commit messages between that hash (exclusive) and HEAD (inclusive)
  const commitMsgsOutput = execSync(`git log ${targetHash}..HEAD --format="%B"`, { encoding: "utf-8" });
