...
function checkGitInstalled(): boolean {
  ...
  try {
    ...
    gitAvailable = false;
    process.stderr.write("[RemoraGitMCP] ERROR: git executable not found in PATH.\n");
  }
  return gitAvailable;
}
...
rl.on("line", (line) => {
  ...
  try {
    ...
    process.stderr.write(`[RemoraGitMCP] Error parsing request JSON: ${e.message}\n`);
  }
});
...
function checkGitRepository(callback: (isRepo: boolean) => void) {
  ...
  if (!fs.existsSync(gitPath)) {
    process.stderr.write("[RemoraGitMCP] WARNING: .git folder not found. Current workspace is not a git repository.\n");
    callback(false);
    return;
  }
  ...
  exec("git rev-parse --is-inside-work-tree", (err, stdout) => {
    ...
    if (!isRepo) {
      process.stderr.write("[RemoraGitMCP] WARNING: git rev-parse failed. Current workspace is not a git repository.\n");
    }
    callback(isRepo);
  });
}
  ...
  if (method === "initialize") {
      ...
      tools: [
        ...
        },
        {
          ...
          inputSchema: {
            type: "object",
            properties: {
              target: { type: "string", description: "The branch name or file path to checkout" }
            },
            required: ["target"]
          }
        },
        {
          ...
          inputSchema: {
            type: "object",
            properties: {
              branch: { type: "string", description: "The source branch name to merge from" }
            },
            required: ["branch"]
          }
        },
    ...
    checkGitRepository((isRepo) => {
      ...
      if (name === "git_status") {
        ...
        if (!target) {
          sendResponse(id, null, { code: -32602, message: "Missing required argument 'target'" });
          return;
        }
        ...
        if (!branch) {
          sendResponse(id, null, { code: -32602, message: "Missing required argument 'branch'" });
          return;
        }
        ...
        if (!message) {
          sendResponse(id, null, { code: -32602, message: "Missing required argument 'message'" });
          return;
        }
      ...
      proc.on("close", (code) => {
        ...
        if (code === 0) {
          ...
          sendResponse(id, {
            content: [{ type: "text", text: `Git exited with code ${code}\n${output}` }]
          });
        }
      });
