...
export function getPlanContent(
	...
	try {
		const row = db
			...
			.get(`artifact_sync_${projectUuid}`) as { content: string } | undefined;
...
export function getPendingEvents(conn?: Database.Database): Array<{
	id: number;
	project_uuid: string;
	event_type: string;
	payload: string;
...
export function markEventProcessed(
	...
	try {
		db.prepare(
			"UPDATE remora_event_queue SET status='processed' WHERE id=?",
		).run(eventId);
	} finally {
		if (ownConn) db.close();
	}
