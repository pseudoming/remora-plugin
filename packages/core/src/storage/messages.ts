...
export function getWatermark(
	...
	try {
		const row = db
			...
			.get(projectUuid, conversationId) as { last_msg_id: number } | undefined;
...
export function getMaxMessageIdUpToLine(
	...
	try {
		const row = db
			...
			.get(conversationId, lineNumber) as { max_id: number | null } | undefined;
		return row && row.max_id ? row.max_id : 0;
	} finally {
		if (ownConn) db.close();
	}
...
export function deleteTopicDecision(
	...
	const ownConn = !conn;
	try {
		db.prepare("DELETE FROM topic_decisions WHERE id=?").run(decisionId);
	} finally {
		if (ownConn) db.close();
	}
}
