	...
	try {
		...
		const rows = db
         ...
         LIMIT ?`,
			...
			.all(projectUuid, convId, convId, `"${safeKeyword}"`, limit) as {
...
function _buildEvidenceTexts(
	...
	return evidenceTexts.length ? ` [证据: ${evidenceTexts.join(" | ")}]` : "";
}
	...
	try {
		...
		const rows = db
         ...
         ORDER BY created_at DESC LIMIT ?`,
			...
			.all(projectUuid, convId, likePattern, likePattern, limit) as Array<{
			topic_id: string;
			decision: string;
			rationale: string;
			evidence_msg_ids: string;
