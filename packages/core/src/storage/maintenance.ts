		...
		try {
           ...
           WHERE pt.source = 'auto' AND pt.status = 'closed'
			...
			for (const row of toDelete) {
				db.prepare(
					"DELETE FROM topic_decisions WHERE project_uuid=? AND topic_id=?",
				).run(row.uuid, row.topic_id);
				db.prepare(
					"DELETE FROM project_topics WHERE uuid=? AND topic_id=?",
				).run(row.uuid, row.topic_id);
				console.log(
           ...
           OR NOT EXISTS (SELECT 1 FROM session_state ss WHERE ss.session_id = w.conversation_id)`,
			...
			if (toDelete.length > 0) {
				...
				for (const [convId, reason] of toDelete) {
					db.prepare("DELETE FROM watermarks WHERE conversation_id=?").run(
						convId,
					);
					db.prepare("DELETE FROM messages WHERE conversation_id=?").run(
						convId,
					);
					db.prepare("DELETE FROM topic_decisions WHERE conversation_id=?").run(
						convId,
					);
					console.log(`[Remora] 水印回收已清除会话 (${reason}): ${convId}`);
				}
				db.prepare("COMMIT").run();
			}
		} finally {
