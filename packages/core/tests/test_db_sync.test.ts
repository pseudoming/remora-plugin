...
describe("test_compactor_db_sync", () => {
	it("compactor db sync", () => {
		...
		const messages = db
			.prepare("SELECT id, line_number FROM messages")
			.all() as Array<{ id: number; line_number: number }>;
		expect(messages.length).toBe(2);
		expect(messages[0].line_number).toBe(1);
		expect(messages[1].line_number).toBe(2);

		// Verify watermark table
		const watermarks_init = db
			.prepare("SELECT last_msg_id FROM watermarks WHERE conversation_id='c1'")
			.all() as Array<{ last_msg_id: number }>;
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
		...
		// Verify decisions
		const decisions = db
			.prepare("SELECT evidence_msg_ids FROM topic_decisions")
			.all() as Array<{ evidence_msg_ids: string }>;
...
describe("test_sync_artifacts_file_changes_artifact", () => {
	it("sync artifacts file changes artifact", () => {
		...
		expect(insertFileChangeCalls.every((c) => c.source === "artifact")).toBe(
			true,
		);
	});
});
