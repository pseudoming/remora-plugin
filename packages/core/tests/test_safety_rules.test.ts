...
describe("PythonCheck", () => {
	it("python -c with pytest denied", () => {
		expect(inspectCommand('python -c "import pytest"')).toEqual([
			"deny",
			"test",
		]);
	});
	...
	it("python -c with unittest denied", () => {
		expect(inspectCommand('python -c "import unittest"')).toEqual([
			"deny",
			"test",
		]);
	});
	...
	it("python3 -c with pytest denied", () => {
		expect(inspectCommand('python3 -c "import pytest"')).toEqual([
			"deny",
			"test",
		]);
	});
	...
	it("python -c safe code allowed", () => {
		expect(inspectCommand("python -c \"print('hello')\"")).toEqual([
			"allow",
			"",
		]);
	});
...
describe("NodeCheck", () => {
	it("node -e with jest denied", () => {
		expect(inspectCommand("node -e \"const jest = require('jest')\"")).toEqual([
			"deny",
			"test",
		]);
	});
	...
	it("node -e with vitest denied", () => {
		expect(
			inspectCommand("node -e \"const vitest = require('vitest')\""),
		).toEqual(["deny", "test"]);
	});

	it("node -e with mocha denied", () => {
		expect(
			inspectCommand("node -e \"const mocha = require('mocha')\""),
		).toEqual(["deny", "test"]);
	});

	it("node -e safe allowed", () => {
		expect(inspectCommand("node -e \"console.log('hi')\"")).toEqual([
			"allow",
			"",
		]);
	});
...
describe("InspectCommandEdgeCases", () => {
	...
	it("shlex failure fallback build match", () => {
		expect(inspectCommand('npm run build "unclosed')).toEqual([
			"deny",
			"build",
		]);
	});
...
describe("AllowSafeCommands", () => {
	...
	it("env var with safe command allowed", () => {
		expect(inspectCommand("MODE=production python script.py")).toEqual([
			"allow",
			"",
		]);
	});
...
describe("PbFileCheck", () => {
	it("denies command with .pb in tokens", () => {
		...
		expect(inspectCommand("python unpack.py --file=my_model.pb")).toEqual([
			"deny",
			"pb_read",
		]);
	});
...
describe("GitCommitEscapeCheck", () => {
	it("allows normal git commit", () => {
		expect(inspectCommand('git commit -m "feat: login functionality"')).toEqual(
			["allow", ""],
		);
	});
	...
	it("denies git commit with consecutive asterisks", () => {
		expect(inspectCommand('git commit -m "feat: login ** critical"')).toEqual([
			"deny",
			"git_escape",
		]);
	});
...
describe("GlobBypassCheck", () => {
	it("denies command with glob metacharacters and sensitive endings", () => {
		expect(inspectCommand("cat scratch/subagent_shared/*.jsonl")).toEqual([
			"deny",
			"glob_bypass",
		]);
		expect(
			inspectCommand(
				"tail -n 50 .system_generated/worktrees/subagent-*/logs/*.log",
			),
		).toEqual(["deny", "glob_bypass"]);
		expect(inspectCommand("cat logs/test?.sqlite")).toEqual([
			"deny",
			"glob_bypass",
		]);
	});
	...
	it("allows command with glob metacharacters but harmless endings", () => {
		...
		expect(inspectCommand("grep 'function' packages/**/*.ts")).toEqual([
			"allow",
			"",
		]);
