...
describe("RuleEngine", () => {
	...
	it("handles neq operator match", () => {
		...
		expect(engine.evaluate({ toolName: "run_command" }, rules).status).toBe(
			"DENY",
		);
		expect(engine.evaluate({ toolName: "view_file" }, rules).status).toBe(
			"ALLOW",
		);
	});
	...
	it("handles contains operator match", () => {
		...
		expect(
			engine.evaluate({ commandLine: "sudo rm -rf /" }, rules).status,
		).toBe("DENY");
		expect(engine.evaluate({ commandLine: "echo hello" }, rules).status).toBe(
			"ALLOW",
		);
	});
	...
	it("handles regex operator match", () => {
		...
		expect(
			engine.evaluate({ path: "/brain/conv1/db.sqlite" }, rules).status,
		).toBe("DENY");
		expect(
			engine.evaluate({ path: "/brain/conv1/db.json" }, rules).status,
		).toBe("ALLOW");
	});
