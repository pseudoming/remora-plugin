...
describe("test_prune_expired_watermarks", () => {
    ...
    `);
		...
		const messages = checkConn
			.prepare("SELECT conversation_id FROM messages ORDER BY conversation_id")
			.all() as Array<{ conversation_id: string }>;
...
describe("test_recall_decisions_edge_cases", () => {
    ...
    `);
		...
		const likeDecisions = dao.recallDecisionsByLike("proj_1", "conv_1", "Test");
...
describe("test_merge_physical_files_to_topic", () => {
	it("test merge physical files to topic", () => {
		...
		dao.mergePhysicalFilesToTopic("proj_1", "topic_A", ["/path/to/file1.py"]);
		row = conn
			.prepare(
				"SELECT associated_files FROM project_topics WHERE uuid='proj_1' AND topic_id='topic_A'",
			)
		...
		dao.mergePhysicalFilesToTopic("proj_1", "topic_A", ["/path/to/file3.py"]);
		row = conn
			.prepare(
				"SELECT associated_files FROM project_topics WHERE uuid='proj_1' AND topic_id='topic_A'",
			)
		...
		conn
			...
			.run(JSON.stringify([{ file: "/path/to/file1.py", source: "auto" }]));
		dao.mergePhysicalFilesToTopic("proj_1", "topic_A", ["/path/to/file1.py"]);
		row = conn
			.prepare(
				"SELECT associated_files FROM project_topics WHERE uuid='proj_1' AND topic_id='topic_A'",
...
describe("test_prune_exception", () => {
	it("test prune exception", () => {
		const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "test_remora_prune_"));
		const origExit = process.exit;
		let exitCalled = false;
		let exitCode: number | null = null;
		process.exit = ((code?: number) => {
