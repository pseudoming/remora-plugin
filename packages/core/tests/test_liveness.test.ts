...
it("cleanSystemReminders with tags", () => {
	expect(
		cleanSystemReminders("hello <system-reminder>foo</system-reminder> world"),
	).toBe("hello  world");
});
...
it("cleanSystemReminders nested tags", () => {
	const text =
		"start <system-reminder>outer<system-reminder>inner</system-reminder></system-reminder> end";
...
it("detectMode alert keyword override", () => {
	...
	expect(detectMode("delete this 草稿", [], alert)).toEqual([
		"alert",
		"delete",
	]);
	expect(detectMode("this is a draft", undefined, ["delete"])).toEqual([
		"relax",
		null,
	]);
});
...
it("detectMode both keyword sets", () => {
	const alert = ["delete"];
	expect(detectMode("delete everything", undefined, alert)).toEqual([
		"alert",
		"delete",
	]);
});
...
describe("TestDetectMode", () => {
	it("alert keyword returns alert with word", () => {
		expect(detectMode("你清醒一点", undefined, ["你清醒一点"])).toEqual([
			"alert",
			"你清醒一点",
		]);
	});
	...
	it("alert overrides relax", () => {
		expect(
			detectMode("讨论方案，你清醒一点", ["讨论"], ["你清醒一点"]),
		).toEqual(["alert", "你清醒一点"]);
	});
	...
	it("multiple alert returns first match", () => {
		expect(detectMode("你好 再见", undefined, ["你好", "再见"])).toEqual([
			"alert",
			"你好",
		]);
	});
	...
	it("case insensitive alert", () => {
		expect(detectMode("UPPERCASE DELETE", undefined, ["delete"])).toEqual([
			"alert",
			"delete",
		]);
	});
