...
describe("normalizeFilepath", () => {
	it("test_target_file", () => {
		expect(
			normalizeFilepath({ TargetFile: "/home/user/project/src/main.py" }),
		).toBe("main.py");
	});

	it("test_absolute_path", () => {
		expect(normalizeFilepath({ AbsolutePath: "/tmp/foo/bar.json" })).toBe(
			"bar.json",
		);
	});
	...
	it("test_target", () => {
		expect(normalizeFilepath({ Target: "relative/path/script.sh" })).toBe(
			"script.sh",
		);
	});
	...
	it("test_falls_through_to_next_alias_if_value_empty", () => {
		expect(
			normalizeFilepath({ TargetFile: "", AbsolutePath: "/b/fallback.md" }),
		).toBe("fallback.md");
	});

	it("test_falls_through_if_value_none", () => {
		expect(normalizeFilepath({ TargetFile: null, FilePath: "/c/real.c" })).toBe(
			"real.c",
		);
	});
	...
	it("test_strips_quotes_from_value", () => {
		expect(normalizeFilepath({ TargetFile: '"/quoted/path/file.py"' })).toBe(
			"file.py",
		);
		expect(normalizeFilepath({ TargetFile: "'single_quoted'" })).toBe(
			"single_quoted",
		);
	});

	it("test_flat_filename", () => {
		expect(normalizeFilepath({ TargetFile: "standalone.py" })).toBe(
			"standalone.py",
		);
	});
});
...
describe("ACTION_PATTERNS", () => {
	...
	it("test_pattern_1_markdown_link_modified", () => {
		...
		const matches = findall(
			p,
			"已修改了 [config.py](file:///home/project/config.py) 文件",
		);
	...
	it("test_pattern_1_markdown_link_created", () => {
		...
		const matches = findall(
			p,
			"成功创建文件 [new_script.sh](file:///tmp/new_script.sh)",
		);
	...
	it("test_pattern_2_in_markdown_link_updated_success", () => {
		...
		const matches = findall(
			p,
			"成功在 [data.json](file:///data.json) 中更新了",
		);
...
describe("ACTION_PATTERNS integration", () => {
	...
	it("test_multipattern_extraction", () => {
		...
		expect(declared).toEqual(
			new Set(["main.py", "utils.js", "helper.ts", "schema.sql"]),
		);
	});

	it("test_no_false_positives_on_prose", () => {
		const text =
			"I think this code looks good. Let's proceed to the next step.";
		const declared = new Set<string>();
		for (const pattern of ACTION_PATTERNS) {
			const matches = findall(pattern, text);
			for (let path of matches) {
